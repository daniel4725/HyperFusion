...
def create_metadata_csv(ADNI_dir, save_path, features_set_idx):
    ...
    features_lst = features_lst + ['IMAGEUID', 'DX_bl']  # add the target and the image id
    adni_csv = adni_csv.loc[:, features_lst]
    adni_csv.loc[adni_csv.DX_bl == "CN", "DX_bl"] = 0
    adni_csv.loc[adni_csv.DX_bl == "LMCI", "DX_bl"] = 1
    adni_csv.loc[adni_csv.DX_bl == "EMCI", "DX_bl"] = 1
    adni_csv.loc[adni_csv.DX_bl == "AD", "DX_bl"] = 2

    adni_csv = preprocess_df_columns(adni_csv, features_preprocess_dict) 

    MASK_PATH_COL = []  # create a mask pth column
    for i in range(len(adni_csv.IMAGEUID)):  # change the image id to the image path in the adni directory
        MASK_PATH_COL.append(os.path.join("ADNI", str(adni_csv.loc[i, "IMAGEUID"]), "brainmask.npy.lz4"))
        adni_csv.loc[i, "IMAGEUID"] = os.path.join("ADNI", str(adni_csv.loc[i, "IMAGEUID"]), "t1p.npy.lz4")
    adni_csv.rename(columns = {'IMAGEUID':'IMAGE_PATH'}, inplace = True)
    adni_csv["MASK_PATH"] = MASK_PATH_COL
...
if __name__ == "__main__":
    ...
    features_set_idx = 8
    adni_images = '1x1x1'  #  '2x2x2'  '1x1x1'

    ADNI_dir = f"/usr/local/faststorage/datasets/adni_class_pred_{adni_images}_v1"
