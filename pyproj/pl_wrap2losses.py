...
class PlModelWrap2losses(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_hat, y_hat2 = self((imgs, tabular))

        loss1 = F.cross_entropy(y_hat, y, weight=self.class_weights.to(self.device))
        loss2 = F.cross_entropy(y_hat2, y, weight=self.class_weights.to(self.device))
        ...
        self.log('train/loss1', loss1, prog_bar=False, on_step=False, on_epoch=True, batch_size=self.batch_size)
        self.log('train/loss2', loss2, prog_bar=False, on_step=False, on_epoch=True, batch_size=self.batch_size)
    ...
    def _shared_eval_step(self, batch, batch_idx, evaluation_type="val"):
        ...
        y_hat1, _ = self((imgs[..., mid_x:], tabular))
        y_hat2, _ = self((imgs[..., :mid_x].flip(dims=(4,)), tabular))
...
class PlModelWrap4test2losses(PlModelWrap2losses):
    def __init__(self, **kwargs):
