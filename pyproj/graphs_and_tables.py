...
def results_df2bars_graph(df, title, metrics, colors, ylim=None):
    ...
    values = np.array(df[metrics])
    std_cols = [metric + '_std' for metric in metrics]
    std_values = np.array(df[std_cols])
...
if __name__ == '__main__':
    df = pd.read_csv('/media/rrtammyfs/Users/daniel/test_results_1-kmodels-precision.csv', index_col=0)
    df = df[df.num_models == 18]
    ...
    # idxs_lst = [i for i in range(df.shape[0])]
    # min_subset_size = 3
    # for subset_size in range(min_subset_size, len(idxs_lst) + 1):
    #     for subset in combinations(idxs_lst, subset_size):
    #         print(subset)

    # df = pd.read_csv('/media/rrtammyfs/Users/daniel/test_results_1.csv', index_col=0)
    # df_val = pd.read_csv('/media/rrtammyfs/Users/daniel/val_results_1.csv', index_col=0)
    ...
    #
    # new_df.to_csv(f'/media/rrtammyfs/Users/daniel/test_results_1-kmodels-{metric2folow}.csv')
