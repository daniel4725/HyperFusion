...
class PlModelWrapBrainAge4Test(PlModelWrapBrainAge):
...
class PlModelWrapADcls2Classes(pl.LightningModule):
    ...
    def _shared_eval_epoch_end(self, step_outputs, type_evaluiation="val"):
        ...
        # mean_val_acc = torch.diag(cm).sum()/len(y)
        CN_acc, AD_acc = torchmetrics.functional.accuracy(y_hat, y, num_classes=self.num_classes, average='none')
        self.log(f'{type_evaluiation}/CN_acc', CN_acc, prog_bar=False, on_step=False, on_epoch=True, batch_size=self.batch_size)
        self.log(f'{type_evaluiation}/AD_acc', AD_acc, prog_bar=False, on_step=False, on_epoch=True, batch_size=self.batch_size)
        self.log(f'{type_evaluiation}/f1_macro', f1_macro, prog_bar=False, on_step=False, on_epoch=True, batch_size=self.batch_size)
        self.log(f'{type_evaluiation}/f1_micro', f1_micro, prog_bar=False, on_step=False, on_epoch=True, batch_size=self.batch_size)
        ...
        threshold = 0.5
        y_pred = (y_hat >= threshold).to(torch.float)
        # Compute confusion matrix
        confusion_matrix = torchmetrics.functional.confusion_matrix(y_pred, y, num_classes=2)
        # # Extract TP, FP, FN, TN
        # TN, FP, FN, TP = confusion_matrix.flatten()
        # # Compute accuracy for AD class
        # AD_acc = TP / (TP + FN)
        AD_acc = (confusion_matrix[0, 0] + confusion_matrix[1, 1]) / len(y_pred)
        print(f'#############################################################')
        print(f'#####################   {confusion_matrix}   ######################')
        print(f'#####################   AD_acc: {AD_acc}   ######################')
        print(f'#############################################################')
...
class PlModelWrapADcls2ClassesBinary(pl.LightningModule):
    def __init__(self, **wrapper_kwargs):
        super().__init__()
        self.save_hyperparameters()
        wrapper_kwargs = EasyDict(self.hparams)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # loss = F.binary_cross_entropy(y_hat.to(torch.float), y.to(torch.float), weight=self.class_weights.to(self.device))
        loss = F.binary_cross_entropy((torch.sigmoid(y_hat.squeeze())).to(torch.float), y.to(torch.float))
        acc = torchmetrics.functional.accuracy((torch.sigmoid(y_hat.squeeze()) > 0.5).to(torch.int), y)
        AUC = torchmetrics.functional.auroc(torch.sigmoid(y_hat), y[:, None])
    ...
    def _shared_eval_step(self, batch, batch_idx, evaluation_type="val"):
        ...
        y_hat2 = self((imgs[..., :mid_x].flip(dims=(4,)), tabular))
        if evaluation_type == 'val':
            y_hat = (torch.sigmoid(y_hat1) + torch.sigmoid(y_hat2))/2
        elif evaluation_type == 'test':
            y_hat = (y_hat1 + y_hat2) / 2
        # y_hat = self((imgs, tabular))
        if evaluation_type == "val":
    ...
    def _shared_eval_epoch_end(self, step_outputs, type_evaluiation="val"):
        ...
        AUC = torchmetrics.functional.auroc(y_hat, y, average='macro')
        specificity = torchmetrics.functional.specificity(y_hat, y)
        sensitivity = torchmetrics.functional.recall(y_hat, y)

        # mean_val_acc = torch.diag(cm).sum()/len(y)
        acc = torchmetrics.functional.accuracy(y_hat, y, average='none')
        self.log(f'{type_evaluiation}/acc', acc, prog_bar=False, on_step=False, on_epoch=True, batch_size=self.batch_size)
        ...
        self.log(f'{type_evaluiation}/best_acc', self.best_acc, prog_bar=True, on_step=False, on_epoch=True, batch_size=self.batch_size)
        self.log(f'{type_evaluiation}/specificity', specificity, prog_bar=True, on_step=False, on_epoch=True, batch_size=self.batch_size)
        self.log(f'{type_evaluiation}/sensitivity', sensitivity, prog_bar=True, on_step=False, on_epoch=True, batch_size=self.batch_size)
...
class PlModelWrapADcls2ClassesBinary4Test(PlModelWrapADcls2Classes):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
